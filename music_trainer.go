		...
		for {
			for k := 0; k < number_of_attacks; k++ {
				...
				for i := 0; i < 2 + third_finger; i++ { //'places fingers on strings'
					for {
						string_num := rand.Int31n(4)	//chooses the string for the finger
...
func guitar_trainer() {
	...
	if chords {
		...
		for {
			for k := 0; k < number_of_attacks; k++ {
				barre := rand_bool(1, 2)
				third_finger := int(rand.Int31n(2))	// 1 if the chord'll use extra finger,
